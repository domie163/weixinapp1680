<template>
	<view class="page_equipment_and_equipment diy_detail diy_detail--new" id="equipment_and_equipment_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					<view v-if="$check_field('get','equipment_coding')" class="col-12 col-6">
						<view class="view">
					...
					<view v-if="$check_field('get','device_name')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>设备名称</span>
							</view>
					...
					<view v-if="$check_field('get','device_status')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>设备状态</span>
							</view>
					...
					<view v-if="$check_field('get','lending_notice')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>出借须知</span>
							</view>
					...
					<view v-if="$check_field('get','usage_duration')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>使用时长</span>
							</view>
					...
					<view v-if="$check_field('get','device_details')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>设备详情</span>
							</view>
		...
		<view class="warp">
			<view class="container-fluid container-fluid-comment--new">
				<view class="row">
					<view class="col">
									<button type="button" class="btn btn-primary" v-if="$check_action('/equipment_borrowing/edit','add')" @click="to_form('/pages/equipment_borrowing/edit')">
							<span>借用</span>
						</button>
								<button type="button" class="btn btn-primary" v-if="$check_action('/record_log/edit','add')" @click="to_form('/pages/record_log/edit')">
							<span>记录</span>
						</button>
								<button type="button" class="btn btn-primary" v-if="$check_action('/repair_and_maintenance_report/edit','add')" @click="to_form('/pages/repair_and_maintenance_report/edit')">
							<span>报修检修</span>
						</button>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=equipment_and_equipment&source_field=equipment_and_equipment_id&source_id=' + obj['equipment_and_equipment_id']">
							评论
						</navigator>
	...
	export default {
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "equipment_and_equipment",
					source_field: "equipment_and_equipment_id",
					source_id: obj["equipment_and_equipment_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "equipment_and_equipment",
					source_field: "equipment_and_equipment_id",
					source_id: this.obj["equipment_and_equipment_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/equipment_and_equipment/set?equipment_and_equipment_id=' + _this.obj["equipment_and_equipment_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
													query.title = this.obj.device_name
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/equipment_and_equipment/set?equipment_and_equipment_id=' + obj["equipment_and_equipment_id"], {
					hits
				}, (res) => {
					if(res.result){
