<template>
	<view class="page_equipment_and_equipment diy_list diy_list_page div_list_tml" id="equipment_and_equipment_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
														<uni-search-bar placeholder="搜索设备名称" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'device_name')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																			<uni-search-bar placeholder="搜索存放位置" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'storage_location')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																		<!-- /搜索栏 -->
						</view>
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?equipment_and_equipment_id=' + o['equipment_and_equipment_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
		...
		data() {
			return {
				url_get_list: "~/api/equipment_and_equipment/get_list?like=0",
											// 设备状态选项列表
				list_device_status: [{value:"全部",text:"全部"}],
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=equipment_and_equipment", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["equipment_and_equipment_id"] === o["source_id"]){
