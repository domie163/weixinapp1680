<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="实验室推荐"
      url="/pages/laboratory/list"
      :list="list_article"
      v-if="$check_action('/laboratory/list', 'get')"
    >
	  <list_laboratory :list="list_laboratory"></list_laboratory>
...
<script>
			...
			import list_laboratory from "@/components/diy/list_laboratory.vue";
  ...
  methods: {
    ...
    toToggle() {
      this.isAdmin = !this.isAdmin;
		...
		*/
		get_laboratory() {
			let url = "~/api/laboratory/get_list?";
									
			this.$get(url, {
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("实验室" ,json.result.list);
					this.list_laboratory = json.result.list;
											this.list_laboratory.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_laboratory ,"laboratory" ,"39956");
									}
			});
		},
