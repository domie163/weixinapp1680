<template>
	<view class="page_experimental_consumables diy_detail diy_detail--new" id="experimental_consumables_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					<view v-if="$check_field('get','consumable_number')" class="col-12 col-6">
						<view class="view">
					...
					<view v-if="$check_field('get','consumable_name')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>耗材名称</span>
							</view>
					...
					<view v-if="$check_field('get','quantity_of_consumables')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>耗材数量</span>
							</view>
					...
					<view v-if="$check_field('get','consumable_price')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>耗材价格</span>
							</view>
					...
					<view v-if="$check_field('get','item_specifications')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>物品规格</span>
							</view>
					...
					<view v-if="$check_field('get','consumable_details')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>耗材详情</span>
							</view>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=experimental_consumables&source_field=experimental_consumables_id&source_id=' + obj['experimental_consumables_id']">
							评论
						</navigator>
	...
	export default {
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "experimental_consumables",
					source_field: "experimental_consumables_id",
					source_id: obj["experimental_consumables_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "experimental_consumables",
					source_field: "experimental_consumables_id",
					source_id: this.obj["experimental_consumables_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/experimental_consumables/set?experimental_consumables_id=' + _this.obj["experimental_consumables_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.consumable_number
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/experimental_consumables/set?experimental_consumables_id=' + obj["experimental_consumables_id"], {
					hits
				}, (res) => {
					if(res.result){
