<template>
	<view class="page_experimental_consumables diy_list diy_list_page div_list_tml" id="experimental_consumables_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
													<uni-search-bar placeholder="搜索耗材编号" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'consumable_number')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																			<uni-search-bar placeholder="搜索耗材名称" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'consumable_name')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																		<!-- /搜索栏 -->
						</view>
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?experimental_consumables_id=' + o['experimental_consumables_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=experimental_consumables", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["experimental_consumables_id"] === o["source_id"]){
