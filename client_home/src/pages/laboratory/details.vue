<template>
	<view class="page_laboratory diy_detail diy_detail--new" id="laboratory_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					<view v-if="$check_field('get','laboratory_number')" class="col-12 col-6">
						<view class="view">
					...
					<view v-if="$check_field('get','laboratory_status')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>实验室状态</span>
							</view>
					...
					<view v-if="$check_field('get','laboratory_name')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>实验室名称</span>
							</view>
					...
					<view v-if="$check_field('get','laboratory_details')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>实验室详情</span>
							</view>
					...
					<view v-if="$check_field('get','location')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>所在位置</span>
							</view>
					...
					<view v-if="$check_field('get','laboratory_equipment')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>实验室设备</span>
							</view>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=laboratory&source_field=laboratory_id&source_id=' + obj['laboratory_id']">
							评论
						</navigator>
	...
	export default {
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					...
					source_field: "laboratory_id",
					source_id: obj["laboratory_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "laboratory",
					source_field: "laboratory_id",
					source_id: this.obj["laboratory_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/laboratory/set?laboratory_id=' + _this.obj["laboratory_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.laboratory_number
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/laboratory/set?laboratory_id=' + obj["laboratory_id"], {
					hits
				}, (res) => {
					if(res.result){
