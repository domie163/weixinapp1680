		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','storage_location')" min-width="200">
					...
					<el-table-column prop="borrowing_users" @sort-change="$sortChange" label="借用用户"
				v-if="user_group == '管理员' || $check_field('get','borrowing_users')" min-width="200">
						<template slot-scope="scope">
					{{ get_user_borrowing_users(scope.row['borrowing_users']) }}
				</template>
					</el-table-column>
					<el-table-column prop="borrowing_time" @sort-change="$sortChange" label="借用时间"
				v-if="user_group == '管理员' || $check_field('get','borrowing_time')" min-width="200">
		                <template slot-scope="scope">
                	{{ $toTime(scope.row["borrowing_time"],"yyyy-MM-dd") }}
                </template>
					</el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','borrowed_quantity')" min-width="200">
					...
					<el-table-column prop="borrowing_status" @sort-change="$sortChange" label="借用状态"
				v-if="user_group == '管理员' || $check_field('get','borrowing_status')" min-width="200">
					</el-table-column>
					<el-table-column prop="borrowing_remarks" @sort-change="$sortChange" label="借用备注"
				v-if="user_group == '管理员' || $check_field('get','borrowing_remarks')" min-width="200">
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/equipment_borrowing/table','set') || $check_action('/equipment_borrowing/view','set') || $check_action('/equipment_borrowing/view','get') || $check_action('/归还/table','add') || $check_action('/归还/view','add')" >
